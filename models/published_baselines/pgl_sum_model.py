...
class SelfAttention(nn.Module):
    def __init__(self, input_size=1024, output_size=1024, freq=10000, heads=1, pos_enc=None):
        ...
        mat_out_size = output_size//heads
        for _ in range(self.heads-1):
            self.Wk.append(nn.Linear(in_features=input_size, out_features=mat_out_size, bias=False))
            self.Wq.append(nn.Linear(in_features=input_size, out_features=mat_out_size, bias=False))
            self.Wv.append(nn.Linear(in_features=input_size, out_features=mat_out_size, bias=False))
        self.Wk.append(nn.Linear(in_features=input_size, out_features=output_size - mat_out_size*(heads-1), bias=False))
        self.Wq.append(nn.Linear(in_features=input_size, out_features=output_size - mat_out_size*(heads-1), bias=False))
        self.Wv.append(nn.Linear(in_features=input_size, out_features=output_size - mat_out_size*(heads-1), bias=False))
...
class PGL_Sum_Adapted(nn.Module):
    def __init__(self,
                 drop_proj:float = 0.1,
                 feat_inp_dim:int = 1664,
                 encoder_type:str = "max",
                 pgl_sum_num_seg:int = 4,
                 pgl_sum_heads:int = 8,
                 pgl_sum_inp_dim:int = 1024,
                 pgl_sum_hid_dim:int = 1024,
                 pgl_sum_fusion_type:str = "Add"):
        super(PGL_Sum_Adapted, self).__init__()
        self.pgl_sum_inp_dim = pgl_sum_inp_dim
        self.pgl_sum_hid_dim = pgl_sum_hid_dim
        self.pgl_sum_num_seg = pgl_sum_num_seg
        self.pgl_sum_fusion_type = pgl_sum_fusion_type
        self.pgl_sum_heads = pgl_sum_heads
        self.encoder_type = encoder_type
        if self.encoder_type in ["max", "avg"]:
            self.shot_enc = mlp_enc(input_size=feat_inp_dim,
                                    d_model=self.pgl_sum_inp_dim,
                                    dropout=drop_proj,
                                    activation="gelu",
                                    pool_type=self.encoder_type,
                                    pool_location=-1)
        else:
            raise ValueError(f"Unknown encoder type: {self.encoder_type}")
        self.pgl_sum = PGL_SUM(input_size=self.pgl_sum_inp_dim,
                               output_size=self.pgl_sum_hid_dim,
                               num_segments=self.pgl_sum_num_seg,
                               heads=self.pgl_sum_heads,
                               fusion=self.pgl_sum_fusion_type)

    def forward(self,
                batched_episodes: torch.Tensor,
                mask: Optional[torch.Tensor] = None):
        batched_shot_feats = self.shot_enc(X=batched_episodes, mask=mask)
        collected_outputs = list()
        for episode in batched_shot_feats:
            out, _ = self.pgl_sum(episode)
...
if __name__ == '__main__':
    # model = PGL_SUM(input_size=256, output_size=256, num_segments=3, fusion="Add")
    # inp = torch.randn(300, 256)
    # output, weights = model(inp)

    mod = PGL_Sum_Adapted(feat_inp_dim=1664,
                          pgl_sum_inp_dim=1024,
                          pgl_sum_hid_dim=1024,
                          pgl_sum_num_seg=4).eval()
    inp = torch.rand(5, 60, 100, 1664)
    mask = torch.ones(5, 60, 100)
    out = mod(inp, mask)
